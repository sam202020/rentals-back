      ...
      if (!i.address) {
        ...
        googleMapsClient
          ...
          .then(response => {
            ...
            return Rental.findByIdAndUpdate(i._id, {
              address
            });
          }).then(record => console.log(record)).catch(err => console.error(err));
      }
    }
  })
}

const convertSting = function (s) {
  if (s) {
    if (s.toLowerCase() === 'a') return 'apartment';
    else if (s.toLowerCase() === 'h') return 'house';
    else if (s.toLowerCase() === 't') return 'townhouse';
    else if (s.toLowerCase() === 'r') return 'room';
  }
}
